...
> 코딩 에이전트의 컨텍스트 창은 작고, 저장소는 큽니다. **vs-token-safer가 그 사이에 들어갑니다.**
>
> 무엇이 어디 있는지, 무엇이 그걸 호출하는지, 이름이 기억나지 않을 땐 "auth 흐름이 어떻게 되더라?"까지
> 물어보세요. 소스를 채팅에 쏟아붓는 대신 짧은 `file:line` 목록으로 답합니다.
>
> - **이름을 알아요?** 정확하게 답합니다. IDE가 쓰는 것과 같은 코드 인덱스를 읽거든요 *(clangd·Roslyn·tsserver·pyright)*.
> - **컴파일러나 프로젝트 설정이 아직 없어요?** 그래도 함수·클래스를 즉시 찾아줍니다. 설정 0.
> - **이름은 까먹고 "무슨 일을 하는지"만 기억나요?** 당신 코드가 이미 쓰는 단어로 찾아줍니다 — AI 모델 없이, 서버로 아무것도 보내지 않고.
> - **코드가 아니라 Markdown·설정 파일이에요?** 헤딩 이름만으로 한 섹션을 읽거나 고칩니다. 파일을 통째로 열 필요 없이.
>
> 수십 MB 에디터·빌드 로그를 채팅에 붙여넣지 않고 읽어 주는 형제 플러그인도 함께 옵니다. **모든 것은 당신의 기기 안에 머뭅니다.**
