...
>
> 될 땐 정확하게 — 진짜 언어 서버(clangd·Roslyn·tsserver·pyright)로. 안 될 땐 그래도 쓸모 있게 — 설치가
> 필요 없는 tree-sitter, 또는 저장소 자체의 단어로 만든 fuzzy 검색(임베딩 없음)으로. Markdown·설정 파일도
> 똑같이, 헤딩 이름만으로 한 섹션을 읽거나 고칩니다. 수십 MB 에디터 로그를 채팅에 붙여넣지 않고 읽어 주는
> 형제 플러그인도 함께 옵니다.
>
> **모든 것은 당신의 기기 안에 머뭅니다.**
