...
**Codex CLI에서** (MCP — 같은 엔진, Claude Code 불필요). 한 줄로 서버를 등록하면 도구가 바로 쓸 수
있습니다 (로컬 전용, 인증/로그인 없음):

```bash
codex mcp add vs-search -- node /ABSOLUTE/PATH/TO/vs-token-safer/server/index.js
```

선택: [adapters/codex/AGENTS.md](adapters/codex/AGENTS.md)를 프로젝트 `AGENTS.md`에 복붙하면 에이전트가 vts
도구를 우선 쓰도록 유도됩니다. 전체 가이드: **[adapters/codex/README.ko.md](adapters/codex/README.ko.md)**
([English](adapters/codex/README.md)). 다른 MCP 호스트 → [adapters/README.md](adapters/README.md).
...
Codex는 MCP를 지원하므로 도구가 그대로 이식됩니다 — 동일한 토큰캡 `file:line` 답변, 전부 로컬. 한 줄로
서버를 등록합니다 (로컬 전용, 인증 없음):

```bash
codex mcp add vs-search -- node /ABSOLUTE/PATH/TO/vs-token-safer/server/index.js
```

선택: [adapters/codex/AGENTS.md](adapters/codex/AGENTS.md)를 `AGENTS.md`에 복붙하면 도구 선택을 유도합니다.
