...
>
> - **정상적으로 빌드되는 상태라면?** 답은 정확합니다 — 에디터가 의존하는 바로 그 코드 인덱스를 읽으니까요.
> - **빌드 환경이 갖춰지지 않았다면?** 그래도 함수와 클래스는 그대로 찾아줍니다. 따로 설정할 것도 없습니다.
> - **이름은 잊었고 무슨 일을 하는지만 떠오른다면?** 코드가 이미 쓰고 있는 단어들로 찾아냅니다 — AI 모델도, 외부 전송도 없습니다.
> - **Markdown이나 설정 파일이라면?** 파일을 통째로 열지 않고 헤딩 이름만으로 원하는 섹션에 바로 갑니다.
>
...
<p align="center">
  ...
  <sub>내장 대시보드(<code>vts serve</code>) — 인덱싱한 저장소를 살아 있는 3D 그래프로, 전부 127.0.0.1에서. 세 가지 모드: <b>include</b> 그래프(clangd), <b>call</b> 그래프(심볼별 실시간 LSP 호출 계층), 그리고 <b>symbol</b> 그래프 — 파일 + import 엣지를 tree-sitter로 그리며 <b>언어 서버가 필요 없어</b> 17개 tree-sitter 언어 어디서든 설정 없이 렌더링됩니다.</sub>
</p>

## 왜 쓰나

- **컨텍스트를 가볍게 유지하세요.** 큰 레포 — TypeScript/Python 모노레포, C#/.NET 솔루션, 26k-TU Unreal C++ 트리까지 — 에서 `grep`은 컨텍스트를 잠식합니다. 언어 서버 인덱스는 토큰 상한이 걸려 약 97~99% 작습니다 ([벤치마크](#성능)).
...
- **변경을 눈으로가 아니라 영향 반경으로 리뷰합니다.** `detect_changes`는 `git diff`를 그것이 바꾼 심볼들로 매핑하고, 각 심볼의 호출자를 따라가 리스크를 **LOW/MED/HIGH**로 채점합니다 — 영향 반경, 호출-연쇄 깊이, 그리고 역사적으로 함께 바뀌던 파일 중 이번 diff가 *빠뜨린* 것으로부터. 리뷰가 "이 변경이 실제로 무엇을 건드리나"에서 출발하며, `file:line`으로 상한이 걸리고, 아무것도 업로드하지 않습니다. 코드-리뷰-컨텍스트 아이디어를 vs-token-safer 방식으로 — **영향 반경은 공식 언어 서버가, 커플링은 당신의 git 이력이 담당하며, 영속 그래프 DB도 임베딩도 없습니다.**
...
- **EXACT** — 이름을 알고 프로젝트가 빌드되는 상태 → 공식 언어 서버(clangd / Roslyn / tsserver / pyright),
  시맨틱의 원천.
- **SYNTACTIC** — 툴체인이 없음 → tree-sitter 파싱(36개 문법 번들, 네이티브 빌드 없음. 그중 19개 언어는 선언
  추출이 튜닝돼 있고 나머지는 제네릭 파싱)이 그래도 진짜 *선언*을 돌려줍니다. 사용처 grep이 아닙니다.
...
**우아한 성능 저하(graceful degradation) — locate는 툴체인이 없거나 차가워도 하드 에러를 내지 않습니다.**
clangd가 **없거나, 차갑거나, 아직 인덱싱 중**일 때 locate는 에러나 차단 대신 한 줄 안내와 함께 SYNTACTIC
(tree-sitter) 계층으로 내려가고, clangd가 워밍되면 EXACT로 다시 올라갑니다. 커밋된 `.vts-index`가 소스와
어긋났으면 답은 그대로 주되 `SYNTACTIC · STALE`로 라벨하고 `climb: vts index` 힌트를 답니다. 그리고 큰
트리에서 콜드 locate가 들어오면 vts가 **백그라운드에서 `.vts-index/`를 자동으로 빌드**하므로 다음 질의
(또는 빌드가 끝난 뒤 같은 세션)는 즉시 답합니다 — 그걸 기다릴 필요가 없습니다. 그래서 아래의 `vts index`는
꼭 외워서 손으로 돌려야 하는 수동 단계가 아니라 *백그라운드에서 자동 빌드되고, 공유하려면 커밋*하는 것으로
보면 됩니다.
...
| `preindex` | `vts preindex` | 첫 질의 전에 clangd 인덱스 빌드 (큰 트리 콜드 스타트). |
| `index` | `vts index` | 커밋 가능한 `.vts-index/` 콜드 스타트 심볼 인덱스 빌드/갱신 (`index --status`로 표시). |
...
| 원시 인덱스 응답 → 캡 목록 (eval, 심볼 1,000개) | ~57,308 토큰 | ~1,515 토큰 | **~97.4%** |
| ~1 MB 에디터 로그 읽기 (`summary`) | ~267,000 토큰 | ~130 토큰 | **~99.95%** |

## 컴패니언: 로컬 모델로 vs-search 구동

**[vts-local-orchestrator](https://github.com/JSungMin/vts-local-orchestrator)** — **어떤 로컬 LLM**(Ollama,
풀-GPU — 모델 불문, 기본값은 벤치마크로 고른 **gemma4:e4b**)이든 바로 이 `vs-search` 도구들을 구동하게 해주는
별도(선택) 컴패니언. Claude는 싸고 양 많은 **코드 위치찾기**를 무료 로컬 모델에 위임하고 압축된 `file:line`
답만 받으므로, 검색 원시 출력이 Claude 컨텍스트에 들어가지 않습니다.

- **CLI**(`qvts`) + 3-way 토큰 절약 패널(이 방식 vs CC가 vs-search 직접 vs CC가 grep)을 갖춘 **라이브 웹
  대시보드** + Claude Code용 위임 라우팅 스킬.
- **토큰 절약 장치**: 영속 절약 장부(`qvts --savings`), locate 캐시(반복은 0-비용), 배치 위임(`--batch`),
  간결한 레포 상대 `file:line` 출력.
- 완전 로컬 — 이 플러그인 헌장과 동일하게 외부 전송 없음.

설치: 이 레포 옆에 클론하고 `npm install` 후 `bash setup-macos.sh`(Windows: `setup.ps1`) 실행 — 설정
스크립트가 GPU·모델과 vs-token-safer·clangd를 자동 탐지합니다. 파이프라인·모델 벤치마크·절약 모델은 그 레포
README 참고.
...
eval(`node eval/run.mjs`, 툴체인 불필요)이 커밋마다 이걸 검증합니다: `~57,308 → ~1,515 토큰` = **97.4%**
(검사 92개).

**구문 계층 vs clangd — 같은 위치를, 콜드 대기 없이.** 또 다른 실제 UE5 게임 모듈(파일 3,143개, 심볼
133,890개, 집계 수치만)에서 tree-sitter 계층은 표본 심볼마다 clangd의 EXACT 답과 **똑같은 `file:line`**을
냈습니다(라인 오차 0). 걸린 시간은 **~240ms** 대 clangd의 콜드 첫 응답 **~127초** — 선언 위치를 찾는 데는
시맨틱 계층의 힘으로도 못 좁히는 ~530배 격차입니다. 그래서 사다리는 clangd가 워밍되는 동안 tree-sitter로
먼저 답하고, warm이 되면 EXACT로 올라갑니다. 인덱스 빌드는 일회 ~62초. 자세한 내용은 [BENCHMARK.md](BENCHMARK.md).
...
**clangd 백그라운드 인덱싱은 트리 크기에 맞춰 조절됩니다(RAM/CPU 티어링).** 거대한 트리가 머신을 잡아먹지
않도록 백그라운드 인덱싱은 번역 단위(TU) 수에 따라 자동 티어링됩니다: TU **4,000개 이하는 FULL**(일반
우선순위), **4k~15k는 SAFE**(유휴 CPU 우선순위로 스로틀), **15,000개 초과는 OFF**(전체 트리 크롤 없음 —
프로젝트 전역 `search_symbol`/`find_references`는 구문 계층으로 폴백, 단일 파일 `read_symbol`/`hover`/아웃라인은
그대로 저비용). 한 줄 안내가 어느 티어인지 알려 주므로 검색이 의도적으로 저하됐음을 압니다. 정해진 해법은
`vts setup --scope <module>` 후 `vts preindex`. 스위치: `VTS_CLANGD_BG_INDEX`(강제 `full`/`off`),
`VTS_CLANGD_BG_INDEX_SOFT_TUS`(`4000`), `VTS_CLANGD_BG_INDEX_HARD_TUS`(`15000`),
`VTS_CLANGD_HUGE_WARM_CAP`(`8`).

**3. 제로-셋업 티어 — 툴체인 없이, 어떤 repo에서나 즉시.** 컴파일 DB도, 언어 서버도, 대기도 없이?
vts는 그래도 **tree-sitter** 파싱으로 `search_symbol`에 답합니다(공식 표준 파서. 36개 문법을 wasm으로 번들
— 네이티브 빌드 없음. 그중 19개 언어는 선언 추출이 튜닝돼 있고 나머지는 제네릭 파싱) — 사용처 grep이 아니라 진짜
*선언*을, 동일한 토큰캡 `file:line` 형태로. 커밋 가능한
...
이걸 손으로 돌릴 일은 드뭅니다: 큰 트리에서 콜드 locate가 들어오면 vts가 **백그라운드에서 `.vts-index/`를
자동 빌드**하므로(`VTS_AUTO_INDEX`, 기본 켜짐) 이후 질의는 즉시입니다. `.vts-index/symbols.jsonl`은 평문·git
커밋 가능·이식성 — 커밋해 두면 팀원(과 본인의 cold start)이 제로 셋업으로 즉시 심볼 검색을 얻습니다. 언어 서버가 인덱싱을 마치면 자동으로 이를 대체합니다(구문 티어는 선언
...
| — | `VTS_CLANGD_BG_INDEX` | auto | clangd 백그라운드 인덱싱을 `full`/`off`로 강제 (기본: 트리 크기로 자동 티어링 — *큰 트리* 참고). |
| — | `VTS_CLANGD_BG_INDEX_SOFT_TUS` | `4000` | 이 TU 수 이하 → FULL 백그라운드 인덱스, 초과 → SAFE(유휴 우선순위, 스로틀). |
| — | `VTS_CLANGD_BG_INDEX_HARD_TUS` | `15000` | 이 TU 수 초과 → 백그라운드 인덱스 OFF(전체 트리 크롤 없음, 구문 계층으로 폴백). |
| — | `VTS_CLANGD_HUGE_WARM_CAP` | `8` | SAFE/OFF 모드의 워밍업 오픈 캡 (큰 트리는 워밍업 자체가 파싱 스파이크). |
...
| — | `VTS_LADDER_LINE` | on (→ `VTS_CERT`) | 한 줄 `[ladder: RUNG — reason. climb: cmd]` 저하 힌트 표시; `0`이면 숨김(미설정이면 `VTS_CERT`를 따름). |
| — | `VTS_AUTO_INDEX` | `1` | 큰 트리에서 콜드 locate가 들어오면 백그라운드로 `.vts-index/` 자동 빌드; `0`이면 끔. |
| — | `VTS_AUTOINDEX_MIN_FILES` | `400` | 백그라운드 자동 인덱스를 유발하는 최소 트리 크기(파일 수). |
| — | `VTS_AUTOINDEX_LOCK_TTL_MS` | `1800000` | 자동 인덱스 빌드의 크로스-프로세스 락 TTL (기본 30분). |
| — | `VTS_STALE_CHECK` | `1` | 어긋난 커밋 인덱스를 `SYNTACTIC · STALE`로 라벨; `0`이면 신선도 검사 생략. |
| — | `VTS_READ_STEER` | `1` | 큰 파일 **Read** 시 `read_symbol`(선언 하나)/심볼편집 도구로 유도; `0`이면 숨김. |
| — | `VTS_READ_STEER_MIN` | `6000` | Read → `read_symbol` 유도를 유발하는 최소 파일 크기(바이트). |
| — | `VTS_ORCH_BLOCK` | `1` | 로컬 오케스트레이터(qvts)가 PATH에 있으면 LOCATE + Bash/Grep 코드검색을 `qvts`로 리다이렉트; `0`이면 경고만. |
| — | `VTS_ORCHESTRATOR_AWARE` | `1` | `0`이면 오케스트레이터 감지 자체를 끔(리다이렉트 미발화). |
| — | `VTS_ORCH_TTL_MS` | `180000` | 동일 locate 재호출이 통과하는 창(위임 후 폴백); 기본 3분. |
| — | `VTS_ORCH_FALLBACK_TTL_MS` | `120000` | 위임 locate가 빈손일 때 Claude가 직접 검색해도 되는 창; 기본 2분. |
</details>
